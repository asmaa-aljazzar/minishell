...
// //     }
// // }
// In your existing execution file (like execution.c)
#include "minishell.h"
...
void main_fork(t_minishell *minishell)
{
    ...
    // Check if command is empty before forking
    if (is_command_empty(minishell->cmd))
    {
        minishell->exit_code = 0;
        return;
    }

    // Handle builtin commands in parent process (not in pipeline)
    if (is_builtin(minishell->cmd))
    {
        // Save original stdin/stdout for builtin redirections
        int saved_stdin = dup(STDIN_FILENO);
        int saved_stdout = dup(STDOUT_FILENO);
