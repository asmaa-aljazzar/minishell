...
void init(t_minishell *ms, char **environ)
{
    ...
    ms->envp = ft_strdup_double(environ); // Initialize the envp array // todo
	ms->env = init_env(&ms, environ); // Initialize the env linked list // todo
    ft_memset(ms->buff, 0, sizeof(ms->buff)); // todo
    increase_SHLVL_var(&ms, ms->env); // Increase SHLVL value by 1 //? Done
}
